...
async function loadData() {
    try {
        // Use relative paths. This works if index.html is at root and data/ is at root.
        const [profile, services, portfolio] = await Promise.all([
            fetchSafe('data/profile.json'),
            fetchSafe('data/services.json'),
            fetchSafe('data/portfolio.json')
        ]);
        ...
        app.innerHTML = `
            <div style="height: 100vh; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; color: #e0e0e0;">
                ...
                <p style="max-width: 600px; line-height: 1.6;">
                    ...
                    Please verify that the <code>data/</code> directory exists and contains valid JSON files.<br>
                    <br>
                ...
                <div style="margin-top: 2rem; font-size: 0.8rem; color: #666;">
                    Diagnostics: Checking ${window.location.href}data/profile.json
                </div>
            </div>
        `;
    }
